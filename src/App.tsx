...
import {
  SITE_ENTERPRISE_URL,
  SITE_GITHUB_ICON_PATH,
  SITE_GITHUB_URL,
  SITE_HOME_URL,
  SITE_NAVIGATION,
} from "./shared/site-navigation";

const GITHUB_URL = SITE_GITHUB_URL;
...
function Header({ page }: { page: Page }) {
  return (
    <header className="zy-site-header">
      <a className="zy-site-brand" href={SITE_HOME_URL} aria-label="知远智能体首页">
        <img src="/zhiyuan-logo.svg" alt="知远" />
        <span>ZHIYUAN AGENT</span>
      </a>
      <nav className="zy-site-nav" aria-label="主导航">
        <div className="zy-site-nav-track">
          {SITE_NAVIGATION.map((item) => (
            <a
              key={item.id}
              href={item.href}
              aria-current={page === "docs" && item.id === "docs" ? "page" : undefined}
            >
              {item.label}
            </a>
          ))}
          <a className="zy-site-enterprise" href={SITE_ENTERPRISE_URL} aria-current={page === "enterprise" ? "page" : undefined}>
            企业版
          </a>
        </div>
      </nav>
      <a className="zy-site-github" href={GITHUB_URL} target="_blank" rel="noreferrer">
        <GithubIcon />
        <span>GitHub</span>
      </a>
    </header>
